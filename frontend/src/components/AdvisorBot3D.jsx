...
/* ── quick-question bank (varied topics) ───────────────────────────────────── */
const QUICK_QUESTIONS = [
  { label: '🌾 NPK Fertilizer',     q: 'What is NPK fertilizer and how do I use it correctly?' },
  { label: '🐛 Aphid Control',      q: 'How to control aphids organically in my field?' },
  { label: '💧 Drip Irrigation',    q: 'What are the benefits of drip irrigation and how to get subsidy?' },
  { label: '🏛️ PM-KISAN Scheme',    q: 'What is PM Kisan scheme, eligibility and how to apply?' },
  { label: '🌱 Improve Soil',       q: 'How to improve soil fertility naturally without chemicals?' },
  ...
  { label: '📋 Fasal Bima Yojana',  q: 'Tell me about Pradhan Mantri Fasal Bima Yojana crop insurance' },
  { label: '🌊 Waterlogged Soil',   q: 'My field has waterlogging problem, how to fix it?' },
  { label: '📈 Increase Yield',     q: 'How to increase crop yield without increasing costs?' },
  { label: '🌰 Groundnut Growing',  q: 'How to grow groundnut — soil, fertilizer and pest management?' },
...
const AdvisorBot3D = ({ isDarkMode = false }) => {
  ...
  /* ── JSX ────────────────────────────────────────────────────────────────── */
  return (
    <div style={{ minHeight:'calc(100vh - 68px)', background:bg,
      ...
      <div style={{ width:'100%', maxWidth:880, display:'flex', flexDirection:'column', gap:14 }}>
        ...
        {/* ── Header ───────────────────────────────────────────────────────── */}
        <div style={{ background:hdrCardBg, backdropFilter:'blur(20px)', border:hdrCardBdr,
          ...
          <div style={{ display:'flex', alignItems:'center', gap:12 }}>
            <div style={{ width:48, height:48, borderRadius:14, flexShrink:0,
              ...
              boxShadow:'0 8px 20px rgba(34,197,94,0.3)' }}>🧑‍🌾</div>
            <div>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:6, marginTop:3 }}>
                ...
                <span style={{ color:'#22c55e', fontSize:'0.72rem', fontWeight:700, letterSpacing:0.8 }}>
                  EXPERT KNOWLEDGE BASE · ALWAYS ACTIVE
                </span>
              </div>
            </div>
          </div>
          <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
            {[
              ...
              <div key={i} style={{ display:'flex', alignItems:'center', gap:5, padding:'5px 10px',
                borderRadius:14, background:statsBg, border:statsBdr, color:statsColor, fontSize:'0.73rem', fontWeight:600 }}>
          ...
          <div style={{ display:'flex', gap:7, flexWrap:'wrap' }}>
            {visibleQuestions.map((item, i) => (
              <button key={i}
                ...
                style={{ padding:'6px 12px', borderRadius:20, border:chipBdr, background:chipBg,
                  color:chipColor, cursor:'pointer', fontSize:'0.76rem', fontWeight:600,
                  transition:'all 0.2s', minHeight:34, touchAction:'manipulation',
                  letterSpacing:0.2 }}>
                {item.label}
              </button>
            ))}
            <button
              ...
              style={{ padding:'6px 12px', borderRadius:20,
                ...
                color:statsColor, cursor:'pointer', fontSize:'0.76rem', fontWeight:600,
                transition:'all 0.2s', minHeight:34, touchAction:'manipulation' }}>
        ...
        {/* ── Chat Window ──────────────────────────────────────────────────── */}
        <div style={{ background:panelBg, backdropFilter:'blur(20px)', border:panelBdr,
          borderRadius:20, overflow:'hidden',
          boxShadow: isDarkMode ? '0 32px 64px rgba(0,0,0,0.4)' : '0 8px 32px rgba(34,197,94,0.12)' }}>

          {/* Messages */}
          <div style={{ padding:'16px', minHeight:320, maxHeight:460, overflowY:'auto',
            display:'flex', flexDirection:'column', gap:12, WebkitOverflowScrolling:'touch' }}>
              ...
              <div key={msg.id} style={{ display:'flex', alignItems:'flex-end', gap:8,
                ...
                <div style={{ maxWidth:'79%' }}>
                  <div style={{
                    ...
                    border: msg.sender === 'user' ? 'none' : msg.isError
                      ? '1px solid rgba(239,68,68,0.3)' : botBubbleBdr,
                    color: msg.sender === 'user' ? '#fff' : msg.isError
                      ? (isDarkMode ? '#fca5a5' : '#dc2626') : botTextColor,
                  ...
                  <div style={{ display:'flex', gap:5, alignItems:'center' }}>
                    {[0, 0.2, 0.4].map((delay, i) => (
                      <div key={i} style={{ width:7, height:7, borderRadius:'50%',
                        background:'#22c55e', animation:`bounce 1.2s ${delay}s infinite` }} />
                    ))}
                    <span style={{ color: isDarkMode ? '#86efac' : '#15803d',
                      fontSize:'0.78rem', marginLeft:8, fontStyle:'italic' }}>Searching knowledge base…</span>
                  </div>
                </div>
              </div>
            )}
          ...
          <div style={{ padding:'12px 14px',
            ...
            <input
              ...
              style={{ flex:1, padding:'12px 18px', borderRadius:28,
            ...
            <button
              ...
              style={{
                ...
                background: loading || !input.trim() ? 'rgba(34,197,94,0.15)' : 'linear-gradient(135deg,#16a34a,#22c55e)',
        ...
        <div style={{ display:'flex', gap:8, justifyContent:'center', flexWrap:'wrap' }}>
          {[
            { icon:'📚', label:'Expert Knowledge Base' },
            { icon:'⚡', label:'Instant Responses' },
            { icon:'🌾', label:'Indian Agriculture Focus' },
            { icon:'🔒', label:'Secure & Private' },
            { icon:'📱', label:'Works on Any Device' },
          ].map((item, i) => (
            <div key={i} style={{ display:'flex', alignItems:'center', gap:5, padding:'5px 12px',
